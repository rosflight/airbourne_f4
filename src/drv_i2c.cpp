	...
	if (sr1 & (I2C_SR1_AF | I2C_SR1_ARLO | I2C_SR1_BERR))
	{
		...
		if (!(sr1 & I2C_SR1_ARLO) && !(dev->CR1 & I2C_CR1_STOP))		// If we dont have an ARLO error, ensure sending of a stop
		{
			if (dev->CR1 & I2C_CR1_START)								// We are currently trying to send a start, this is very bad as start,stop will hang the peripheral
			{
				...
				i2cInit(dev);											// Reset and configure the hardware
			} 
			else
			{
...
void I2C::handle_event()
{
	...
	if (sr1 & I2C_SR1_SB)			// EV5 (in ref manual) - Start just sent
	{
		...
		if (reading_ && subaddress_sent_)
		{
            ...
            I2C_Send7bitAddress(I2Cx_, addr_, I2C_Direction_Receiver);	// start a read
		}
		else
		{
	...
	{
		final_stop_ = true;	//THIS??
		if (reading_ && subaddress_sent_) // EVT7_2, EVT7_3
		{
			if (len_ > 2)
			{
				data_buffer_[index_++] = I2C_RecieveData(dev); //read N-2
				len_--;
				I2C_GenerateSTOP(dev, ENABLE);
				final_stop_ = true;
				data_buffer_[index_++] = I2C_RecieveData(dev); //read N-1
				len--;
				I2C_ITConfig(dev, I2C_IT_BUF, ENABLE);	//enable for last EV7
			}
			else 
			{
				if (final_stop_)
				...
				index++; //indicates job completeion THIS???///////////////
			}
		}
		else // EVT8_2 - either subaddress sent or write completion
		{
			...
			else //send the subaddress
			{
				I2C_GenerateSTART(dev, ENABLE);
				subaddress_sent_ = true;
			}
		}
		while (dev->CR1 & I2C_CR1_START); // wait for start to clear, otherwise you get constant BTF
	}

	else if (sr1 & I2C_SR1_RXNE)	// EV7 - byte received
	{
		
	}

	else if (sr1 & I2C_SR1_TXE)		// EV8, EV8_1 - byte transmitted
	{
